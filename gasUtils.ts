...
export function setStoragePrices(configRaw: Cell, prices: StorageValue) {
    const config = configRaw.beginParse().loadDictDirect(Dictionary.Keys.Int(32), Dictionary.Values.Cell());
    const storageData = Dictionary.loadDirect(Dictionary.Keys.Uint(32),storageValue, config.get(18)!);
    const keys = storageData.keys();

    if (keys.length === 0) {
        throw new Error("Storage prices dictionary is empty");
    }

    const maxKey = Math.max(...keys);
    storageData.set(maxKey, prices);
...
export function getGasePrices(configRaw: Cell) {
    const config = configRaw.beginParse().loadDictDirect(Dictionary.Keys.Int(32), Dictionary.Values.Cell());
    const storageData = Dictionary.loadDirect(Dictionary.Keys.Uint(32),storageValue, config.get(18)!);
    const values      = storageData.values();
    return values[values.length - 1];
...
export function getDueLimits(configRaw: Cell, workchain: 0 | -1): DueLimits {
    const config = configRaw.beginParse().loadDictDirect(Dictionary.Keys.Int(32), Dictionary.Values.Cell());
    ...
    */
    ...
    const gas_price =  ds.loadUintBig(64);
    const gas_limit =  ds.loadUintBig(64);
    const special_gas_limit =  ds.loadUintBig(64);
    const gas_credit =  ds.loadUintBig(64);
    const block_gas_limit =  ds.loadUintBig(64);
    const freeze_due_limit =  ds.loadUintBig(64);
    const delete_due_limit =  ds.loadUintBig(64);
